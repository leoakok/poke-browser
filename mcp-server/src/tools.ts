...
async function writeScreenshotFallbackFile(base64: string, format: "png" | "jpeg"): Promise<string> {
...
export function registerTools(mcp: McpServer): void {
  mcp.registerTool(
    ...
    async () => {
      ...
      return { content: [{ type: "text" as const, text: BROWSER_GUIDE_MARKDOWN }] };
    }
  );
    ...
    "type_text",
    {
      ...
      inputSchema: {
        ...
        selector: z
          ...
          .optional()
          .describe(
            "Optional target selector. Use only after verifying focus and only when selector matches the real active editor. Safer default: omit selector after click+focus check."
          ),
        x: z
          .number()
          .optional()
          .describe(
            "Optional viewport X for visual cursor feedback marker only. Does not determine typing target."
          ),
        y: z
          .number()
          .optional()
          .describe(
            "Optional viewport Y for visual cursor feedback marker only. Does not determine typing target."
          ),
        tabId: tabIdSchema.optional().describe("Optional target tab id; defaults to active tab."),
        clear: z
          .boolean()
          .optional()
          .describe(
            "If true (default), clear existing value before typing. If false, append/insert at current caret/selection in the active editor."
          ),
      },
    },
  ...
  mcp.registerTool(
    ...
    async ({ tabId, format, quality, uploadUrl }): Promise<CallToolResult> => {
      ...
      const resolvedUpload =
        (typeof uploadUrl === "string" && uploadUrl.trim() !== "" ? uploadUrl.trim() : undefined) ??
      ...
      try {
        const result = await bridge.request(
          "screenshot",
          { tabId, format: imageFormat, quality },
          PENDING_REQUEST_TIMEOUT_MS
        );
        if (!isScreenshotResultPayload(result)) {
          return toolError("Extension returned an invalid screenshot payload.");
        ...
        const fallback = async (): Promise<CallToolResult> => {
          const localPath = await writeScreenshotFallbackFile(base64, imageFormat);
          return toolText({ success: false as const, base64, localPath });
        };
        ...
        if (parsed) {
          return toolText({ success: true as const, mediaId: parsed.mediaId, url: parsed.url });
        }
        return fallback();
      } catch (e) {
        if (e instanceof RateLimitError) {
